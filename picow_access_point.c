...
static int   test_server_content(const char *request,
        const char                          *params,
        char                                *result,
    ...
    if (strncmp(request, LED_TEST, sizeof(LED_TEST) - 1) == 0) {
        ...
        // Generate result
        if (led_state) {
            len = snprintf(result, max_result_len, LED_TEST_BODY, "ON", 0,
                    "OFF");
        } else {
            len = snprintf(result, max_result_len, LED_TEST_BODY, "OFF", 1,
                    "ON");
        }
    }
    return (len);
...
err_t tcp_server_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p,
    ...
    if (p->tot_len > 0) {
        ...
        pbuf_copy_partial(p,
                con_state->headers,
                p->tot_len
                > sizeof(con_state->headers) - 1 ? sizeof(con_state->headers)
                - 1 : p->tot_len,
                0);
        // Handle GET request
        if (strncmp(HTTP_GET, con_state->headers, sizeof(HTTP_GET) - 1) == 0) {
            ...
            // Generate content
            con_state->result_len = test_server_content(request,
                    params,
                    con_state->result,
            ...
            if (con_state->result_len) {
                err = tcp_write(pcb, con_state->result, con_state->result_len,
                        0);
                if (err != ERR_OK) {
...
static err_t tcp_server_poll(void *arg, struct tcp_pcb *pcb) {
    ...
    return (tcp_close_client_connection(con_state, pcb, ERR_OK)); // Just
                                                                  // disconnect
                                                                  // clent?
}
...
// This "worker" function is called to safely perform work when instructed by
// key_pressed_func
void                               key_pressed_worker_func(
        async_context_t             *context,
...
int                                main() {
    ...
    async_context_add_when_pending_worker(cyw43_arch_async_context(),
            &key_pressed_worker);
